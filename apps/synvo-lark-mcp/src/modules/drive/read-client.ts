...
export const larkBatchMetadataDocumentLimit = 200;
...
export class LarkDriveReader implements DriveReader {
  ...
  async listFolderPage(input: {
    ...
    if (nextPageToken !== undefined && nextPageToken === input.pageToken) {
      malformedResponse("Lark returned a repeated Drive pagination cursor.");
    }
  ...
  async getMetadata(input: {
    ...
    if (input.documents.length > larkBatchMetadataDocumentLimit) {
      throw driveToolError(
        "LIMIT_EXCEEDED",
        "The Drive metadata request exceeds Lark's read-only batch limit.",
      );
    }
    assertUniqueTokens(
      input.documents.map((document) => document.token),
      "The Drive metadata request contained duplicate identifiers.",
    );
    if (
      input.documents.some(
        (document) => document.token.length === 0 || document.type.length === 0,
      )
    ) {
    ...
    const body = await this.#requestJson(url, {
      ...
      body: JSON.stringify({
        request_docs: input.documents.map((document) => ({
    ...
    const metas = parsed.data.data.metas;
    assertUniqueTokens(
      metas.map((metadata) => metadata.doc_token),
      "Lark returned duplicate Drive metadata identifiers.",
    );
    const expected = new Map(
      input.documents.map((document) => [document.token, document.type]),
    );
    if (
      metas.length !== expected.size ||
      metas.some(
        (metadata) => expected.get(metadata.doc_token) !== metadata.doc_type,
      )
    ) {
      malformedResponse("Lark returned incomplete or unexpected Drive metadata.");
    }

    return metas.map((metadata) => ({
...
const internalListPageSchema = z.object({
  items: z.array(
    z.object({
      token: nonemptyStringSchema,
      name: nonemptyStringSchema,
      type: nonemptyStringSchema,
      parentToken: nonemptyStringSchema,
      createdTime: nonemptyStringSchema.optional(),
      modifiedTime: nonemptyStringSchema.optional(),
      ownerId: nonemptyStringSchema.optional(),
    }),
  ),
  hasMore: z.boolean(),
  nextPageToken: nonemptyStringSchema.optional(),
});
...
export async function listFolderCompletely(
  ...
  for (let page = 0; page < maxPages; page += 1) {
    ...
    const rawResult: unknown = await reader.listFolderPage({
      accessToken: input.accessToken,
      folderToken: input.folderToken,
      pageSize,
      pageToken,
    });
    const parsed = internalListPageSchema.safeParse(rawResult);
    if (!parsed.success) {
      throw driveToolError(
        "MALFORMED_RESPONSE",
        "Lark returned a malformed Drive listing.",
      );
    }
    ...
    if (result.items.length > pageSize) {
      throw driveToolError(
        "MALFORMED_RESPONSE",
        "Lark returned more Drive items than requested.",
      );
    }
    if (result.hasMore !== (result.nextPageToken !== undefined)) {
      throw driveToolError(
        "INCOMPLETE_SCAN",
        "Lark returned inconsistent Drive pagination data.",
      );
    }
    if (
    ...
    for (const item of result.items) {
      if (item.parentToken !== input.folderToken) {
        throw driveToolError(
          "MALFORMED_RESPONSE",
          "Lark returned a Drive item outside the requested folder.",
        );
      }
      if (seenItemTokens.has(item.token)) {
        throw driveToolError(
          "INCOMPLETE_SCAN",
          "Lark returned duplicate Drive item identifiers.",
        );
      }
      seenItemTokens.add(item.token);
    }

    if (items.length + result.items.length > maxItems) {
      throw driveToolError(
        "LIMIT_EXCEEDED",
