...
import { createAssistantWebHandler } from "./server.js";

async function withServer(
  handler: ReturnType<typeof createAssistantWebHandler>,
...
test("exposes the MCP route only when an endpoint is configured", async () => {
  const baseOptions = {
    oauthService: {
      async beginAuthorization() {
        throw new Error("unused");
  ...
  await withServer(createAssistantWebHandler(baseOptions), async (origin) => {
  ...
  await withServer(handler, async (origin) => {
    assert.equal((await fetch(`${origin}/mcp`)).status, 204);
